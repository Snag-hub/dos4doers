...
export async function GET(request: Request) {
  try {
    ...
    for (const user of subscribers) {
      ...
      // We attempt to update the timestamp ONLY if it's old or null.
      const today = new Date();
      today.setHours(0, 0, 0, 0); // Start of today (Server Time)
      ...
      const updateResult = await db.update(users)
        ...
        .where(
          and(
            eq(users.id, user.id),
            or(
              isNull(users.lastDailyDigestAt),
              lt(users.lastDailyDigestAt, today)
            )
          )
        )
        .returning({ id: users.id });

      // If no rows were returned, it means the condition failed -> Already sent today (or running concurrently)
      if (updateResult.length === 0) {
        console.log(`⏭️  [DIGEST] Skipping ${user.email} - Already sent today (Locked by another process)`);
        continue;
      }
      ...
      try {
        ...
        if (upcomingMeetings.length === 0 && dueReminders.length === 0 && newInboxItems.length === 0) {
          ...
          await db.update(users)
            .set({ lastDailyDigestAt: user.lastDailyDigestAt })
        ...
        console.error(`❌ [DIGEST] Error processing ${user.email}, releasing lock`, error);
        // Error occurred (e.g. email failed send), releasing lock data so we can try again next run
        await db.update(users)
          .set({ lastDailyDigestAt: user.lastDailyDigestAt })
          .where(eq(users.id, user.id));
      }
